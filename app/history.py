...
def render_history(db):
    """
    Render a two-column interview history view in Streamlit.
    
    Displays past interviews as selectable buttons in a left column and shows details plus questions & answers for the selected interview in a right column. If no interviews exist, shows an informational message and a Back button. Selecting an interview sets the selected interview in Streamlit session state; the Back controls clear the view selection and toggle the history view.
    
    Parameters:
        db: A database-like object with methods `get_all_interviews()` and `get_questions(interview_id)` used to retrieve interview records and their associated questions.
    
    Side effects:
        - Reads and writes `st.session_state.selected_interview_id` and `st.session_state.show_history`.
        - May call `st.rerun()` to refresh the app.
    """
